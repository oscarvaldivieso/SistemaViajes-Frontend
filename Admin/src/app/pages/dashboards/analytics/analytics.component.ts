...
import { statData, widget2 } from './data';
...
export class AnalyticsComponent {
  ...
  //Llamando al endpoint de topVuelos
  this.http.get<any[]>('https://localhost:7091/ListarTopVuelosReservados')
      .subscribe(data => {
        const categorias = data.map(v => v.vuel_Nombre); // <-- Aquí el cambio
        const cantidades = data.map(v => v.totalReservaciones);
        this._totalrevenueChart('["--tb-primary","--tb-light" ]', categorias, cantidades);
      });


      //Llamando al endpoint de topHoteles
  this.http.get<any[]>('https://localhost:7091/ListarTopHotelesReservados')
  .subscribe(data => {
    const categorias = data.map(v => v.htel_Nombre); // <-- Aquí el cambio
    const cantidades = data.map(v => v.totalReservaciones);
    this._topHotelesChart('["--tb-secondary","--tb-light" ]', categorias, cantidades);
  });

  //Llamando al endpoint de topHoteles
  this.http.get<any[]>('https://localhost:7091/ListarTopRestaurantesReservados')
      .subscribe(data => {
        const categorias = data.map(v => v.rest_Nombre); // <-- Aquí el cambio
        const cantidades = data.map(v => v.totalReservaciones);
        this._topRestaurantesChart('["--tb-dark","--tb-light" ]', categorias, cantidades);
      });
    ...
    setTimeout(() => {
      ...
      pointSeries.pushDataItem({ latitude: 14.0723, longitude: -87.1921}, {name: "Tegucigalpa"}); // Tegucigalpa
      pointSeries.pushDataItem({ latitude: 15.5050, longitude: -88.0250 }, {name: "San Pedro Sula"}); // San Pedro Sula
      pointSeries.pushDataItem({ latitude: 15.7631, longitude: -86.7964 }, {name: "La Ceiba"}); // La Ceiba
      pointSeries.pushDataItem({ latitude: 13.3007, longitude: -87.1908 }, {name: "Choluteca"}); // Choluteca
      pointSeries.pushDataItem({ latitude: 14.4513, longitude: -87.6376 }, {name: "Comayagua"}); // Comayagua
    }, 0);
  ...
  */
  private _totalrevenueChart(colors: any, categories: string[], seriesData: number[]) {
      ...
      yaxis: {
        labels: {
          ...
          formatter: function (y: any) {
            return y.toFixed(0); // Sin la "k", ya que son cantidades reales
          }
        },
      },
      legend: {
  ...
  * Top Hoteles Charts
  */
  private _topHotelesChart(colors: any, categories: string[], seriesData: number[]) {
    colors = this.getChartColorsArray(colors);
    this.topHotelesChart = {
      series: [{
        name: 'Reservaciones',
        data: seriesData
      }],
...
/**
  * Top Restaurantes Charts
  */
private _topRestaurantesChart(colors: any, categories: string[], seriesData: number[]) {
  colors = this.getChartColorsArray(colors);
  this.topRestaurantesChart = {
    series: [{
      name: 'Reservaciones',
      data: seriesData
    }],
...
*/
  ...
  private _mainChart(colors: any) {
    ...
    this.mainChart = {
      ...
      yAxis: {
        type: 'category',
        data: ['San Pedro Sula', 'Tegucigalpa', 'La Ceiba', 'Comayagua', 'Islas de la Bahia'],
        inverse: true,
